...
class VariableNode(GenericNode):
    ...
    def getmessagetocheck(self, checkneighborid=None):
        ...
        if checkneighborid is None:
            ...
            return None
        states = np.sum(np.array(states,dtype=np.ndarray))
        #print("Hello",states)
...
class CheckNodeBinary(GenericNode):
    ...
    def getmessagetovar(self, varneighborid):
        ...
        if varneighborid is None:
            ...
            return None
        #print("Second",type(states))
        #print("Second",(states))
        states=2*np.arctanh(np.prod(np.tanh(np.array(states,dtype=np.float64)/2)))
        return states
    ...
    def updatechecks(self, checknodelist=None):
        ...
        for checknodeid in checknodelist:
            ...
            for varnodeid in varneighborlist:
                ...
                checknode.setmessagefromvar(varnodeid,self.getvarnode(varnodeid).getmessagetocheck(checknode.id))
